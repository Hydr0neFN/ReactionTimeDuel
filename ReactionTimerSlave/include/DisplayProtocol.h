...
#define DISP_GAME_START     0x31  // Game starting (after pairing complete)
#define DISP_PLAYER_READY   0x32  // Player got ready & assigned: DATA_LOW = player number (1-4)
#define DISP_COUNTDOWN      0x33  // DATA_LOW = seconds (3, 2, 1)
#define DISP_GO             0x34  // Round start - show "GO!"
...
void loop() {
  if (receivePacket(DisplaySerial, rxPacket)) {
    ...
    switch (cmd) {
      ...
      case DISP_GAME_START:
        // Game is starting (pairing complete, transitioning to gameplay)
        break;

      case DISP_PLAYER_READY:
        // A player got ready and was assigned a number
        // dataLow = player number (1-4)
        // Show their player circle as "ready" (e.g., green)
        break;
